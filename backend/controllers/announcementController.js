...
const getRecentAnnouncements = async (req, res) => {
  try {
    ...
    const announcements = await Announcement.findRecent(limit, req.user.role);
    console.log(`[DEBUG] Recent announcements for role ${req.user.role}:`, announcements.length);
    res.json(announcements);
  } catch (error) {
